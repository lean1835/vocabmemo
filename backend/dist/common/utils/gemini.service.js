...
class GeminiService {
    ...
    constructor() {
        ...
        this.genAI = new generative_ai_1.GoogleGenerativeAI(environment_1.GEMINI_API_KEY);
    }
    ...
    async analyzeVocabulary(input, preferredModel) {
        ...
        for (let i = 0; i < candidateModels.length; i++) {
            ...
            try {
                logger_1.logger.info(`[Gemini AI] Thử phân tích bằng model ${modelName} cho: "${cleanInput}"`);
                return await this.executeAnalysis(cleanInput, modelName);
            }
            catch (err) {
                logger_1.logger.warn(`[Gemini AI] Model ${modelName} báo lỗi (${err.message}). Tự động chuyển đổi sang model tiếp theo...`);
            }
        }
        // Nếu tất cả các model đều thất bại
        logger_1.logger.error(`[Gemini AI] Tất cả các model Gemini đều báo lỗi hoặc hết hạn mức! Kích hoạt chế độ sinh dữ liệu dự phòng offline (Local Fallback) cho từ: "${cleanInput}"`);
